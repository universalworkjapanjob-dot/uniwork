...
function renderDaily() {
  ...
  activeUsers.forEach(u => {
    ...
    const startTime = rec ? rec.startTime : '';
    const endTime = rec ? rec.endTime : '';
...
function updateDailyWage(userId) {
  ...
  if (['attend', 'farm', 'half_am', 'half_pm'].includes(status)) {
    if (start && end) {
      ...
      if (status === 'half_am' || status === 'half_pm') hours = 2;
    }
  ...
  document.getElementById('d-hours-'+userId).textContent = hours ? hours+'h' : '-';
...
function saveDaily() {
  ...
  activeUsers.forEach(u => {
    ...
    let hours = 0, wage = 0;
    if (['attend', 'farm', 'half_am', 'half_pm'].includes(status) && startTime && endTime) {
      const [sh, sm] = startTime.split(':').map(Number);
      const [eh, em] = endTime.split(':').map(Number);
      const mins = (eh*60+em) - (sh*60+sm);
      hours = Math.floor(mins / 30) * 0.5;
      if (hours < 0) hours = 0;
      wage = Math.round(u.wageCommute * hours);
    } else if (status === 'home') {
      hours = u.maxHours;
      wage = Math.round(u.wageHome * hours);
    }
...
function renderMonthly() {
  ...
  thead += '<th>通所</th><th>在宅</th><th>欠席</th><th>出勤率</th><th>工賃</th></tr>';
  
  let tbody = '';
  activeUsers.forEach(u => {
    let att=0, home=0, abs=0, cancel=0, wage=0;
    ...
    for (let d = 1; d <= days; d++) {
      ...
      else if (rec.status==='attend'||rec.status==='half_am'||rec.status==='half_pm') { att++; wage+=rec.wage||0; cell='<span class="att-attend">○</span>'; }
      else if (rec.status==='home') { home++; wage+=rec.wage||0; cell='<span class="att-home">在</span>'; }
      else if (rec.status==='farm') { att++; wage+=rec.wage||0; cell='<span class="att-farm">農</span>'; }
    ...
    const total = att+home+abs+cancel;
    const rate = total > 0 ? Math.round((att+home)/total*100) : 0;
    const rateColor = rate>=80?'#2e7d32':rate>=60?'#f57f17':'#c62828';
    tbody += `<td class="center">${att}</td><td class="center">${home}</td><td class="center">${abs}</td>
      <td class="center" style="color:${rateColor};font-weight:700;">${rate}%</td>
      <td style="text-align:right;color:#2e7d32;font-weight:700;">¥${wage.toLocaleString()}</td></tr>`;
  });
...
function exportMonthlyCSV() {
  ...
  let rows = [['氏名', ...Array.from({length:days},(_,i)=>i+1+'日'), '通所', '在宅', '欠席', '出勤率', '工賃']];
  activeUsers.forEach(u => {
    let row = [u.name];
    let att=0, home=0, abs=0, wage=0;
    for (let d = 1; d <= days; d++) {
      const dateStr = `${ym}-${String(d).padStart(2,'0')}`;
      const rec = monthRecs.find(r => r.userId===u.id && r.date===dateStr);
      if (!rec) { row.push(''); continue; }
      if (['attend','half_am','half_pm','farm'].includes(rec.status)) { att++; wage+=rec.wage||0; row.push('○'); }
      else if (rec.status==='home') { home++; wage+=rec.wage||0; row.push('在'); }
      else if (rec.status==='absent') { abs++; row.push('×'); }
      else if (rec.status==='cancel') { row.push('ｷｬ'); }
      else row.push('');
    }
    const total = att+home+abs;
    const rate = total > 0 ? Math.round((att+home)/total*100)+'%' : '-';
    row.push(att, home, abs, rate, wage);
    rows.push(row);
  });
...
function renderWages() {
  ...
  const stats = activeUsers.map(u => {
    ...
    const attDays = ur.filter(r=>['attend','half_am','half_pm','farm'].includes(r.status)).length;
    const homeDays = ur.filter(r=>r.status==='home').length;
    const attHours = ur.reduce((s,r)=>['attend','half_am','half_pm','farm'].includes(r.status)?s+(r.hours||0):s, 0);
    ...
    return { user: u, attDays, homeDays, attHours, homeHours, wage, prevWage };
  });
