...
function calcHoursAndWage(status, start, end, user) {
  ...
  if (['attend', 'farm', 'half_am', 'half_pm'].includes(status)) {
    const wageRate = status === 'farm' ? user.wageCommute : user.wageCommute;
    if (start && end) {
      ...
      const actualMins = hours * 60;
      if (actualMins < stdMins) {
        const diff = stdMins - actualMins;
...
function renderMonthly() {
  ...
  thead += '<th>通所日</th><th>通所時間</th><th>通所工賃</th><th>在宅日</th><th>在宅時間</th><th>在宅工賃</th><th>農場日</th><th>農場時間</th><th>農場工賃</th><th>欠席</th><th>ｷｬﾝｾﾙ</th><th>出勤率</th><th>工賃合計</th></tr>';
  ...
  activeUsers.forEach(u => {
    ...
    for (let d = 1; d <= days; d++) {
      ...
      else if (rec.status==='attend'||rec.status==='half_am'||rec.status==='half_pm') { attDays++; attHours+=rec.hours||0; attWage+=rec.wage||0; cell='<span class="att-attend">○</span>'; }
      else if (rec.status==='home') { homeDays++; homeHours+=rec.hours||0; homeWage+=rec.wage||0; cell='<span class="att-home">在</span>'; }
      else if (rec.status==='farm') { farmDays++; farmHours+=rec.hours||0; farmWage+=rec.wage||0; cell='<span class="att-farm">農</span>'; }
...
function exportMonthlyCSV() {
  ...
  let rows = [['氏名', ...Array.from({length:days},(_,i)=>i+1+'日'), '通所日','通所時間','通所工賃','在宅日','在宅時間','在宅工賃','農場日','農場時間','農場工賃','欠席','ｷｬﾝｾﾙ','出勤率','工賃合計']];
  activeUsers.forEach(u => {
    let row = [u.name];
