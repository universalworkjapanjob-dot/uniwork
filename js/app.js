...
function calcHoursAndWage(status, start, end, user) {
  ...
  if (['attend', 'half_am', 'half_pm'].includes(status)) {
    ...
    const stdH = parseFloat(user.maxHours) || 0;
    if (start && end) {
      const [sh, sm] = start.split(':').map(Number);
      const [eh, em] = end.split(':').map(Number);
      let mins = (eh*60+em) - (sh*60+sm);
...
function updateDailyWage(userId) {
  ...
  const noteHtml = note ? `<span style="font-size:11px;color:#e65100;">${note}</span>` : '';
  document.getElementById('d-hours-'+userId).innerHTML =
    hours > 0 ? `<span style="font-weight:700;">${hours}h</span>${noteHtml}` : '-';
  document.getElementById('d-wage-'+userId).textContent = wage > 0 ? '¥'+wage.toLocaleString() : '-';
}
