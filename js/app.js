    ...
    for (let d = 1; d <= days; d++) {
      ...
      if (rec && rec.status) {
        if (rec.status==='absent') { abs++; cell='<span class="att-absent">×</span>'; }
          ...
          if (rec.status.startsWith('att_')) { attD++; attH+=aH; attW+=aW; cell='<span class="att-attend">○</span>'; }
          else if (rec.status.startsWith('home_')) { homeD++; homeH+=aH; homeW+=aW; cell='<span class="att-home">在</span>'; }
          else if (rec.status.startsWith('farm_')) { farmD++; farmH+=aH; farmW+=aW; cell='<span class="att-farm">農</span>'; }
...
function renderAnalytics() {
  ...
  if (analyticsTab === 'overview') {
    ...
    setTimeout(() => {
      ...
      monthRecs.forEach(r => { if(r.status.startsWith('att')) typeCount['通所']++; else if(r.status.startsWith('home')) typeCount['在宅']++; else if(r.status.startsWith('farm')) typeCount['農場']++; });
    ...
    const stats = activeUsers.map(u => {
      const ur = monthRecs.filter(r=>r.userId===u.id), att = ur.filter(r=>['attend','home','farm'].includes(r.status)||r.status.includes('_')).length;
    ...
    monthRecs.forEach(r => { const dow = new Date(r.date).getDay(); if (dow>=1 && dow<=5) { if(r.status==='absent') absentD[dow-1]++; else attendD[dow-1]++; } });
