...
function renderDaily() {
  ...
  activeUsers.forEach(u => {
    ...
    const schDefaults = sch ? {
        attend: { status:'att_4h',  start:'10:00', end:'14:00' },
      am:     { status:'att_am',  start:'10:00', end:'12:00' },
      pm:     { status:'att_pm',  start:'13:00', end:'15:00' },
      home:   { status:'home_4h', start:'10:00', end:'14:00' },
      farm:   { status:'farm_4h', start:'10:00', end:'14:00' },
      cancel: { status:'cancel',  start:'',      end:''      },
      full:   { status:'att_4h',  start:'10:00', end:'14:00' },
    ...
    html += `<tr>
      ...
      <td>
        <select id="d-status-${u.id}" class="form-control" style="min-width:150px;" onchange="applyStatusDefaults('${u.id}')">
          <option value="" ${status===''?'selected':''}>未入力</option>
          <optgroup label="通所">
            <option value="att_am"  ${status==='att_am' ?'selected':''}>通所 午前（2h）</option>
            <option value="att_pm"  ${status==='att_pm' ?'selected':''}>通所 午後（2h）</option>
            <option value="att_4h"  ${status==='att_4h' ?'selected':''}>通所 終日4h</option>
            <option value="att_5h"  ${status==='att_5h' ?'selected':''}>通所 終日5h</option>
          </optgroup>
          <optgroup label="在宅">
            <option value="home_am" ${status==='home_am'?'selected':''}>在宅 午前（2h）</option>
            <option value="home_pm" ${status==='home_pm'?'selected':''}>在宅 午後（2h）</option>
            <option value="home_4h" ${status==='home_4h'?'selected':''}>在宅 終日4h</option>
            <option value="home_5h" ${status==='home_5h'?'selected':''}>在宅 終日5h</option>
          </optgroup>
          <optgroup label="農場">
            <option value="farm_am" ${status==='farm_am'?'selected':''}>農場 午前（2h）</option>
            <option value="farm_pm" ${status==='farm_pm'?'selected':''}>農場 午後（2h）</option>
            <option value="farm_4h" ${status==='farm_4h'?'selected':''}>農場 終日4h</option>
            <option value="farm_5h" ${status==='farm_5h'?'selected':''}>農場 終日5h</option>
          </optgroup>
          <optgroup label="その他">
...
function calcSlotHW(status, start, end, user) {
  ...
  const rate = isHome ? (user.wageHome||0) : (user.wageCommute||0);
...
function updateDailyTotal(userId) {
  ...
  document.getElementById('d-wage-'+userId).textContent  = totalW > 0 ? '¥'+totalW.toLocaleString() : '-';
}
    ...
    for (let d = 1; d <= days; d++) {
      ...
      if (dow===0||dow===6) { cls='background:#f5f5f5;'; cell='<span style="color:#ddd;">—</span>'; }
        ...
        function slotH(s, st, en) {
          if (!s || s==='absent' || s==='cancel') return 0;
          if (st && en) {
        ...
        const attH  = slotH(amS.startsWith('att_') ?amS:'', rec.amStart, rec.amEnd)
                    + slotH(pmS.startsWith('att_') ?pmS:'', rec.pmStart, rec.pmEnd);
        const homeH = slotH(amS.startsWith('home_')?amS:'', rec.amStart, rec.amEnd)
                    + slotH(pmS.startsWith('home_')?pmS:'', rec.pmStart, rec.pmEnd);
        const farmH = slotH(amS.startsWith('farm_')?amS:'', rec.amStart, rec.amEnd)
                    + slotH(pmS.startsWith('farm_')?pmS:'', rec.pmStart, rec.pmEnd);
        // 延べ人数：1日1カウント（午前通所+午後在宅でも各区分1回まで）
        if (attH  > 0) { attDays++;  attHours  += attH; }
        if (homeH > 0) { homeDays++; homeHours += homeH; }
        if (farmH > 0) { farmDays++; farmHours += farmH; }
        // 工賃はrecに合算済み
        const totalRecWage = rec.wage||0;
        if (attH>0||homeH>0||farmH>0) { attWage += totalRecWage; }
        // セル表示
        const parts=[];
        if(amS&&amS!=='absent'&&amS!=='cancel') parts.push(`<span class="${amS.startsWith('att_')?'att-attend':amS.startsWith('home_')?'att-home':'att-farm'}">${amS.startsWith('att_')?'○':amS.startsWith('home_')?'在':'農'}</span>`);
        if(pmS&&pmS!=='absent'&&pmS!=='cancel') parts.push(`<span class="${pmS.startsWith('att_')?'att-attend':pmS.startsWith('home_')?'att-home':'att-farm'}">${pmS.startsWith('att_')?'○':pmS.startsWith('home_')?'在':'農'}</span>`);
        cell=parts.join('');
      }
...
function exportMonthlyCSV() {
  ...
  activeUsers.forEach(u => {
    ...
    for (let d = 1; d <= days; d++) {
      ...
      if (rec.status.startsWith('att_'))  { attDays++;  attHours+=rec.hours||0;  attWage+=rec.wage||0;  row.push('○'+(rec.hours||'')); }
      else if (rec.status.startsWith('home_')) { homeDays++; homeHours+=rec.hours||0; homeWage+=rec.wage||0; row.push('在'+(rec.hours||'')); }
      else if (rec.status.startsWith('farm_')) { farmDays++; farmHours+=rec.hours||0; farmWage+=rec.wage||0; row.push('農'+(rec.hours||'')); }
