...
function getScheduleLabel(type) {
  const map = { am: '<span class="sc-am">午前</span>', pm: '<span class="sc-pm">午後</span>', full: '<span class="sc-full">終日4h</span>', full5: '<span class="sc-full">終日5h</span>', home: '<span class="sc-home">在宅</span>', farm: '<span class="sc-farm">農場</span>', cancel: '<span class="sc-cancel">ｷｬ</span>', none: '' };
  return map[type] || '';
}
...
function renderDaily() {
  ...
  activeUsers.forEach(u => {
    ...
    // 出勤予定から午前・午後の初期ステータスを決定（実績未入力時のみ）
    ...
    const schMap = {
      ...
      home:  { am:'home_am', amS:'10:00', amE:'12:00', pm:'home_pm',pmS:'13:00', pmE:'15:00' },
      farm:  { am:'farm_am', amS:'10:00', amE:'12:00', pm:'farm_pm',pmS:'13:00', pmE:'15:00' },
      cancel:{ am:'cancel',  amS:'',      amE:'',      pm:'cancel', pmS:'',      pmE:''      },
    };
    const sd = sch ? (schMap[sch.type] || {}) : {};

    const amStatus = rec ? (rec.amStatus||'') : (sd.am||'');
    const amStart  = rec ? (rec.amStart ||'') : (sd.amS||'');
    const amEnd    = rec ? (rec.amEnd   ||'') : (sd.amE||'');
    const pmStatus = rec ? (rec.pmStatus||'') : (sd.pm||'');
    const pmStart  = rec ? (rec.pmStart ||'') : (sd.pmS||'');
    const pmEnd    = rec ? (rec.pmEnd   ||'') : (sd.pmE||'');
    const absReason = rec ? (rec.absenceReason||'') : '';
    const notes     = rec ? (rec.notes||'') : '';

    function statusOpts(sel) {
      return [
        ['','未入力'],['att_am','通所'],['home_am','在宅'],['farm_am','農場'],['absent','欠席'],['cancel','ｷｬﾝｾﾙ']
      ].map(([v,l])=>`<option value="${v}" ${sel===v?'selected':''}>${l}</option>`).join('');
    }

    html += `<tr>
      <td><strong>${u.name}</strong></td>
      <td><select id="d-am-${u.id}" class="form-control" style="min-width:80px;" onchange="updateDailyTotal('${u.id}')">${statusOpts(amStatus)}</select></td>
      <td><input type="time" id="d-ams-${u.id}" value="${amStart}" class="form-control" style="width:88px;" onchange="updateDailyTotal('${u.id}')"></td>
      <td><input type="time" id="d-ame-${u.id}" value="${amEnd}"   class="form-control" style="width:88px;" onchange="updateDailyTotal('${u.id}')"></td>
      <td><select id="d-pm-${u.id}" class="form-control" style="min-width:80px;" onchange="updateDailyTotal('${u.id}')">${statusOpts(pmStatus)}</select></td>
      <td><input type="time" id="d-pms-${u.id}" value="${pmStart}" class="form-control" style="width:88px;" onchange="updateDailyTotal('${u.id}')"></td>
      <td><input type="time" id="d-pme-${u.id}" value="${pmEnd}"   class="form-control" style="width:88px;" onchange="updateDailyTotal('${u.id}')"></td>
      ...
      <td><input type="text" id="d-reason-${u.id}" value="${absReason}" class="form-control" style="width:100px;" placeholder="理由"></td>
      <td><input type="text" id="d-notes-${u.id}"  value="${notes}"     class="form-control" style="width:100px;" placeholder="備考"></td>
    </tr>`;
  });
...
function calcSlotHW(status, start, end, user) {
  ...
  if (start && end) {
    ...
    hours = mins > 0 ? Math.floor(mins/30)*0.5 : 0;
  } else {
    // 時間未入力は計算しない
...
function updateDailyTotal(userId) {
  ...
  if (!user) return;
  const amSt = document.getElementById('d-am-'+userId).value;
  const amS  = document.getElementById('d-ams-'+userId).value;
  const amE  = document.getElementById('d-ame-'+userId).value;
  const pmSt = document.getElementById('d-pm-'+userId).value;
  const pmS  = document.getElementById('d-pms-'+userId).value;
  const pmE  = document.getElementById('d-pme-'+userId).value;
  const am = calcSlotHW(amSt, amS, amE, user);
  const pm = calcSlotHW(pmSt, pmS, pmE, user);
...
function calcHoursAndWage(status, start, end, user) {
  const r = calcSlotHW(status, start, end, user);
  return { hours: r.hours, wage: r.wage, note: '' };
}
...
function saveDaily() {
  ...
  activeUsers.forEach(u => {
    const amEl = document.getElementById('d-am-'+u.id);
    if (!amEl) return;
    const amStatus = amEl.value;
    const pmStatus = document.getElementById('d-pm-'+u.id).value;
    const amStart  = document.getElementById('d-ams-'+u.id).value;
    const amEnd    = document.getElementById('d-ame-'+u.id).value;
    const pmStart  = document.getElementById('d-pms-'+u.id).value;
    const pmEnd    = document.getElementById('d-pme-'+u.id).value;
    const absenceReason = document.getElementById('d-reason-'+u.id).value;
    const notes    = document.getElementById('d-notes-'+u.id).value;
    const am = calcSlotHW(amStatus, amStart, amEnd, u);
    const pm = calcSlotHW(pmStatus, pmStart, pmEnd, u);
    const hours = Math.round((am.hours + pm.hours)*10)/10;
    const wage  = am.wage + pm.wage;
    const status = amStatus || pmStatus || '';
    const existing = DB.records.findIndex(r => r.date===dateStr && r.userId===u.id);
    const rec = { id: uid(), date: dateStr, userId: u.id,
      amStatus, amStart, amEnd, pmStatus, pmStart, pmEnd,
      status, hours, wage, absenceReason, notes };
...
function renderMonthly() {
  ...
  activeUsers.forEach(u => {
    ...
    for (let d = 1; d <= days; d++) {
      ...
      // 旧ステータス互換（attend/home/farm/half_am/half_pm）
      else if (['attend','half_am','half_pm'].includes(rec.status)) {
        const h = rec.hours||0;
        const std = parseFloat(u.maxHours)||0;
        const diff = Math.round((std - h)*10)/10;
        attDays++; attHours+=h; attWage+=rec.wage||0;
        const deductLabel = diff>0 ? `<br><span style="color:#e65100;font-size:10px;">-${diff}h</span>` : '';
        cell=`<span class="att-attend">○${h>0?h+'h':''}</span>${deductLabel}`;
      }
      else if (rec.status==='home') {
        const h = rec.hours||0;
        homeDays++; homeHours+=h; homeWage+=rec.wage||0;
        cell=`<span class="att-home">在${h>0?h+'h':''}</span>`;
      }
      else if (rec.status==='farm') {
        const h = rec.hours||0;
        const std = parseFloat(u.maxHours)||0;
        const diff = Math.round((std - h)*10)/10;
        farmDays++; farmHours+=h; farmWage+=rec.wage||0;
        const deductLabel = diff>0 ? `<br><span style="color:#e65100;font-size:10px;">-${diff}h</span>` : '';
        cell=`<span class="att-farm">農${h>0?h+'h':''}</span>${deductLabel}`;
      }
      else if (rec.amStatus || rec.pmStatus) {
        const amS = rec.amStatus||''; const pmS = rec.pmStatus||'';
        // スロット時間計算
        function slotH(s, st, en) {
          if (!s || s==='absent' || s==='cancel' || s==='') return 0;
          if (st && en) {
            const [sh,sm]=st.split(':').map(Number), [eh,em]=en.split(':').map(Number);
            const m=(eh*60+em)-(sh*60+sm); return m>0?Math.floor(m/30)*0.5:0;
          }
          return 2;
        }
        // 区分別時間
        const aH = slotH(amS.startsWith('att_') ?amS:'', rec.amStart, rec.amEnd)
                 + slotH(pmS.startsWith('att_') ?pmS:'', rec.pmStart, rec.pmEnd);
        const hH = slotH(amS.startsWith('home_')?amS:'', rec.amStart, rec.amEnd)
                 + slotH(pmS.startsWith('home_')?pmS:'', rec.pmStart, rec.pmEnd);
        const fH = slotH(amS.startsWith('farm_')?amS:'', rec.amStart, rec.amEnd)
                 + slotH(pmS.startsWith('farm_')?pmS:'', rec.pmStart, rec.pmEnd);
        // 区分別工賃
        const aW = Math.round((u.wageCommute||0) * aH);
        const hW = Math.round((u.wageHome||0)    * hH);
        const fW = Math.round((u.wageFarm||u.wageCommute||0) * fH);
        // 延べ人数：同区分は1日1カウント
        if (aH > 0) { attDays++;  attHours  += aH; attWage  += aW; }
        if (hH > 0) { homeDays++; homeHours += hH; homeWage += hW; }
        if (fH > 0) { farmDays++; farmHours += fH; farmWage += fW; }
        // セル表示（実勤時間＋遅刻表示）
        const std = parseFloat(u.maxHours)||0;
        const totalH = Math.round((aH+hH+fH)*10)/10;
        const diff = Math.round((std - totalH)*10)/10;
        const deductLabel = diff>0 ? `<br><span style="color:#e65100;font-size:10px;">-${diff}h</span>` : '';
        const parts=[];
        if(amS&&amS!=='absent'&&amS!=='cancel'&&amS!=='') {
          const lbl = amS.startsWith('att_')?'○':amS.startsWith('home_')?'在':'農';
          const cls2 = amS.startsWith('att_')?'att-attend':amS.startsWith('home_')?'att-home':'att-farm';
          parts.push(`<span class="${cls2}">${lbl}</span>`);
        }
        if(pmS&&pmS!=='absent'&&pmS!=='cancel'&&pmS!=='') {
          const lbl = pmS.startsWith('att_')?'○':pmS.startsWith('home_')?'在':'農';
          const cls2 = pmS.startsWith('att_')?'att-attend':pmS.startsWith('home_')?'att-home':'att-farm';
          parts.push(`<span class="${cls2}">${lbl}</span>`);
        }
        cell = parts.join('')+`<br><span style="font-size:10px;">${totalH}h</span>`+deductLabel;
      }
      tbody += `<td class="center" style="${cls}">${cell}</td>`;
    }
    // 控除時間：当月の全実績で（保存済みhours合計 vs 出勤日数×利用者maxHours）
    const userRecs = monthRecs.filter(r => r.userId===u.id);
    userRecs.forEach(r => {
      if (r.status==='absent'||r.status==='cancel') return;
      const std = parseFloat(u.maxHours)||0;
      const act = r.hours||0;
      const diff = Math.round((std - act)*10)/10;
      if (diff > 0) deductHours = Math.round((deductHours + diff)*10)/10;
    });
...
function exportMonthlyCSV() {
  ...
  activeUsers.forEach(u => {
    ...
    for (let d = 1; d <= days; d++) {
      ...
      if (rec.status.startsWith('att_')||['attend','half_am','half_pm'].includes(rec.status))  { attDays++;  attHours+=rec.hours||0;  attWage+=rec.wage||0;  row.push('○'+(rec.hours||'')); }
      else if (rec.status.startsWith('home_')||rec.status==='home') { homeDays++; homeHours+=rec.hours||0; homeWage+=rec.wage||0; row.push('在'+(rec.hours||'')); }
      else if (rec.status.startsWith('farm_')||rec.status==='farm') { farmDays++; farmHours+=rec.hours||0; farmWage+=rec.wage||0; row.push('農'+(rec.hours||'')); }
    ...
    row.push(attDays,attHours+'h',attWage,homeDays,homeHours+'h',homeWage,farmDays,farmHours+'h',farmWage,abs,cancel,'-',rate,attWage+homeWage+farmWage);
    rows.push(row);
  });
...
function openUserModal(id) {
  ...
  openModal(`
    ...
    <div class="modal-body">
      ...
      <div class="form-group"><label>時給（農場）円</label><input type="number" id="u-wage-f" class="form-control" value="${u?u.wageFarm||0:0}"></div>
...
function saveUser(id) {
  const data = {
    ...
    wageFarm: parseInt(document.getElementById('u-wage-f').value)||0,
...
async function downloadAppZip() {
  ...
  try {
    ...
    for (const f of files) {
      // キャッシュを無視して最新ファイルを取得
      const res  = await fetch(f.fetchUrl + '?v=' + Date.now(), { cache: 'no-store' });
